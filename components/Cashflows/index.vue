<template>
  <section class="w-100 user-wrap">
    ...
    <el-table :data="cashflowsList" border size="small" max-height="700">
      <el-table-column prop="full_name" label="来源" width="160">
      ...
      <el-table-column prop="type" label="收入方式" width="140">
        <template slot-scope="scope">
          {{ getCashflowsType(scope.row.type)['name'] }}
        </template>
      </el-table-column>
      <el-table-column prop="target_platform_wallet_id" label="目标平台钱包ID" width="160">
        <template slot-scope="scope">
          <el-popover
            v-if="scope.row.target_platform_wallet_id"
            placement="top-start"
            width="200"
            trigger="hover"
            :content="scope.row.target_platform_wallet_id"
          >
            <p slot="reference" class="title-line">
              {{ scope.row.target_platform_wallet_id }}
            </p>
          </el-popover>
        </template>
      </el-table-column>
      <el-table-column prop="target_wallet_id" label="目标钱包ID" width="160">
        <template slot-scope="scope">
          <el-popover
            v-if="scope.row.target_wallet_id"
            placement="top-start"
            width="200"
            trigger="hover"
            :content="scope.row.target_wallet_id"
          >
            <p slot="reference" class="title-line">
