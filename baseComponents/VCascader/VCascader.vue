<template>
  <section>
    <v-menu :content-class="'cascader-wrap'" v-model="showMenu" :close-on-content-click="false" offset-y tile>
  ...
  computed: {
    inputValue() {
      const input = this.selectedItems
        ...
        .join(this.seperator);
        if(input === '' && this.items.length > 0 && this.items[0][this.itemValue] === null){
          return this.items[0][this.itemText]
        }else{
          return input;
        }
    }
